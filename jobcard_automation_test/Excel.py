...
def create_cardjob_object(sheet,jobcard):
    ...
    for row in range(2, max_row + 1):
        ...
        tab_job = sheet.cell(row,6).value
        category_job = sheet.cell(row,7).value
        new_card_job = CardJob(card_name = card_name, start_date=start_date,end_date = end_date, card_status=card_status,tab_job=tab_job,category_job=category_job)
